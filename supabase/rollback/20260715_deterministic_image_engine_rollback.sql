-- ============================================================================
-- Deterministic Image Engine — ROLLBACK
-- ============================================================================
-- Reverses the three 20260715* migrations in reverse order. Restores the
-- previous get_seo_page_render body byte-for-byte (from migration
-- 20260712093500_render_rpc_exposes_indexability.sql), then removes the
-- engine functions, indexes, triggers, constraints and columns.
--
-- Data note: dropping the added image_library / seo_page_images / seo_pages
-- columns discards any values written into them (entity tags, priorities,
-- assignment modes, materialization metadata). No pre-existing column or row
-- is touched. Rows inserted with source='materialized' are deleted BEFORE the
-- source column is dropped so no cache row survives as a fake manual mapping.
-- ============================================================================

begin;

-- ----------------------------------------------------------------------------
-- 1. Restore the previous render RPC (pre-engine version)
-- ----------------------------------------------------------------------------
create or replace function public.get_seo_page_render(p_slug text)
returns jsonb
language plpgsql
stable
set search_path = public
as $function$
declare
  result jsonb;
begin
  select jsonb_build_object(
    'page', jsonb_build_object(
      'id', p.id,
      'slug', p.slug,
      'title', p.title,
      'h1', p.h1,
      'meta_title', p.meta_title,
      'meta_description', p.meta_description,
      'layout_variant_id', p.layout_variant_id,
      'robots', p.robots,
      'is_indexable', (p.robots like 'index%')
    ),

    'section_blocks', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'id', cb.id,
          'section_key', cb.block_key,
          'block_key', cb.block_key,
          'title', cb.heading,
          'heading', cb.heading,
          'body', cb.body,
          'cta_label', cb.cta_label,
          'cta_url', cb.cta_url,
          'image_url', cb.image_url,
          'image_alt', cb.image_alt,
          'position', cb.display_order
        )
        order by cb.display_order
      )
      from seo_content_blocks cb
      where cb.page_id = p.id
    ), '[]'::jsonb),

    'assigned_images', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'id', spi.id,
          'section_name', spi.section_name,
          'display_order', spi.display_order,
          'is_primary', spi.is_primary,
          'image_id', il.id,
          'image_key', il.image_key,
          'image_url', il.image_url,
          'alt', il.alt_template,
          'image_type', il.image_type,
          'section_target', il.section_target,
          'width', il.width,
          'height', il.height
        )
        order by spi.section_name, spi.display_order
      )
      from seo_page_images spi
      join image_library il on il.id = spi.image_id
      where spi.page_id = p.id
        and spi.is_active = true
        and il.is_active = true
    ), '[]'::jsonb),

    'faqs', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'id', f.id,
          'question', f.question,
          'answer', f.answer,
          'priority', f.priority,
          'display_order', f.display_order,
          'include_in_schema', f.include_in_schema
        )
        order by f.display_order, f.priority desc
      )
      from seo_faqs f
      where f.is_active = true
        and (
          f.product_entity_id is null
          or f.product_entity_id = p.product_entity_id
        )
        and (
          f.country_entity_id is null
          or f.country_entity_id = p.country_entity_id
        )
        and (
          f.buyer_type_entity_id is null
          or f.buyer_type_entity_id = p.buyer_type_entity_id
        )
    ), '[]'::jsonb),

    'products', coalesce((
      select jsonb_agg(jsonb_build_object(
        'sort_order',  pp.display_order,
        'is_featured', pp.is_featured,
        'slug',        e.slug,
        'title',       coalesce(pp.card_title, e.name),
        'description', coalesce(pp.card_description, e.attributes->>'card_description'),
        'cta_text',    pp.cta_text,
        'moq',         coalesce(pp.moq, nullif(e.attributes->>'moq', '')::int),
        'image_url',   coalesce(pp.image_override_url, e.attributes->>'image_url')
      ) order by pp.display_order)
      from seo_page_products pp
      join seo_entities e on e.id = pp.product_entity_id
      where pp.page_id = p.id
    ), '[]'::jsonb),

    'internal_links', coalesce((
      select jsonb_object_agg(g.link_group, g.links)
      from (
        select l.link_group,
               jsonb_agg(jsonb_build_object(
                 'anchor',     coalesce(l.anchor_text, tp.title),
                 'slug',       tp.slug,
                 'sort_order', l.display_order
               ) order by l.display_order) as links
        from seo_internal_links l
        join seo_pages tp on tp.id = l.target_page_id
        where l.source_page_id = p.id
          and tp.status = 'published'
        group by l.link_group
      ) g
    ), '{}'::jsonb)
  )
  into result
  from seo_pages p
  where p.slug = p_slug
    and p.status = 'published'
  limit 1;

  return result;
end;
$function$;

-- ----------------------------------------------------------------------------
-- 2. Drop the engine functions
-- ----------------------------------------------------------------------------
drop function if exists public.materialize_seo_page_images(bigint);
drop function if exists public.debug_seo_image_candidates(bigint, text, boolean);
drop function if exists public.resolve_seo_page_image(bigint, text, boolean, integer);
drop function if exists public.render_image_alt(text, bigint, text);

-- ----------------------------------------------------------------------------
-- 3. seo_pages — remove assignment mode
-- ----------------------------------------------------------------------------
alter table public.seo_pages
  drop constraint if exists seo_pages_image_assignment_mode_check;
alter table public.seo_pages
  drop column if exists image_assignment_mode;

-- ----------------------------------------------------------------------------
-- 4. seo_page_images — remove engine columns (cache rows first)
-- ----------------------------------------------------------------------------
delete from public.seo_page_images where source = 'materialized';
alter table public.seo_page_images
  drop constraint if exists seo_page_images_source_check;
drop index if exists public.seo_page_images_page_source_idx;
drop index if exists public.seo_page_images_image_idx;
drop index if exists public.seo_pages_meta_image_idx;
alter table public.seo_page_images
  drop column if exists source,
  drop column if exists resolver_version,
  drop column if exists selection_score,
  drop column if exists selection_reason,
  drop column if exists materialized_at;

-- ----------------------------------------------------------------------------
-- 5. image_library — remove engine columns, indexes, triggers, constraints
-- ----------------------------------------------------------------------------
drop trigger if exists image_library_validate_entities on public.image_library;
drop function if exists public.validate_image_library_entities();
drop trigger if exists image_library_set_updated_at on public.image_library;

drop index if exists public.image_library_resolver_idx;
drop index if exists public.image_library_compat_sections_gin;
drop index if exists public.image_library_country_idx;
drop index if exists public.image_library_state_idx;
drop index if exists public.image_library_city_idx;
drop index if exists public.image_library_product_idx;
drop index if exists public.image_library_industry_idx;
drop index if exists public.image_library_buyer_type_idx;
drop index if exists public.image_library_material_idx;
drop index if exists public.image_library_service_idx;
drop index if exists public.image_library_default_idx;

alter table public.image_library
  drop constraint if exists image_library_geographic_scope_check,
  drop constraint if exists image_library_valid_window_check;

alter table public.image_library
  drop column if exists product_entity_id,
  drop column if exists country_entity_id,
  drop column if exists state_entity_id,
  drop column if exists city_entity_id,
  drop column if exists industry_entity_id,
  drop column if exists buyer_type_entity_id,
  drop column if exists material_entity_id,
  drop column if exists service_entity_id,
  drop column if exists geographic_scope,
  drop column if exists compatible_sections,
  drop column if exists country_code,
  drop column if exists state_code,
  drop column if exists city_slug,
  drop column if exists priority,
  drop column if exists quality_score,
  drop column if exists specificity_score,
  drop column if exists is_default,
  drop column if exists is_demo,
  drop column if exists valid_from,
  drop column if exists valid_until,
  drop column if exists license_type,
  drop column if exists source_name,
  drop column if exists source_url,
  drop column if exists copyright_owner,
  drop column if exists updated_at;

commit;
