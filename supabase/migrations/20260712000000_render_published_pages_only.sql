-- Fix: get_seo_page_render must only return published pages.
--
-- Audit finding (2026-07-12): the live function's page lookup was
--   `where p.slug = p_slug limit 1` with NO status filter, so draft and
-- archived pages were publicly renderable by anyone who knew the slug
-- (RLS is disabled on seo_pages and anon can execute the RPC).
--
-- This migration:
--   1. adds `and p.status = 'published'` to the page lookup
--   2. pins `search_path = public` (advisor WARN: mutable search_path)
--
-- The JSON response contract is unchanged: page, section_blocks,
-- assigned_images, faqs, products, internal_links. The function stays
-- STABLE, SECURITY INVOKER, and callable by anon (grants are preserved
